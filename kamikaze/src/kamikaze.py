...
from mavros_msgs.srv import WaypointPush, WaypointClear, CommandLong, WaypointClear, WaypointSetCurrent
import math
...
class UAVWaypointManager:
    def __init__(self):
        ...
        self.loiter_lat = 36.9398208
        self.loiter_lon = 35.5604845
        self.loiter_alt = 70

        # Flight parameters
        self.road_distance = 300

        self.set_params_for_dive_service = rospy.Service(
            "set_params_for_dive", Trigger, self.set_params_for_dive)

        self.set_params_for_safe_fly_service = rospy.Service(
            "set_params_for_safe_fly", Trigger, self.set_params_for_safe_fly)

        self.abort_service = rospy.Service(
            "abort_kamikaze", Trigger, self.abort_kamikaze)

        self.get_param_service = rospy.Service(
            "get_param", Trigger, self.get_param)
    ...
    def calculate_waypoint_coordinates(self, radii, azimuth_angle, altitude):
        ...
        enu_coords.append(
            (exit_x+target_x, exit_y+target_y, 20))

        exit_x2, exit_y2 = utils.calculate_coordinates(
            large_radius - 20, azimuth_angle)
    ...
    def create_waypoints(self, geodetic_coords):
        ...
        wp_loiter = Waypoint()
        wp_loiter.frame = Waypoint.FRAME_GLOBAL_REL_ALT
        wp_loiter.command = 17
        wp_loiter.param3 = 40
        wp_loiter.is_current = True
        wp_loiter.autocontinue = True
        wp_loiter.x_lat = self.loiter_lat
        wp_loiter.y_long = self.loiter_lon
        wp_loiter.z_alt = self.loiter_alt

        waypoints.waypoints.append(wp_loiter)
        print(waypoints)
        return waypoints
    ...
    def handle_start_mission(self, req):
        ...
        self.azimuth_angle = rospy.get_param("azimuth_angle", 180)
        self.dive_angle = math.radians(rospy.get_param("dive_angle", 62))
        self.approach_distance = rospy.get_param("approach_distance", 85)
        self.circle_offset = rospy.get_param("circle_offset", -50)
        ...
        while 1:
            if self.abort:
                ...
                waypoint_set_current_service = rospy.ServiceProxy(
                    '/mavros/mission/set_current', WaypointSetCurrent)
                response = waypoint_set_current_service(6)
                self.abort = 0
                return TriggerResponse(success=1)
            if self.last_rec_wp == 2 and not self.qr_started:
                rospy.wait_for_service("/start_Qr")
                qr_start = rospy.ServiceProxy('/start_Qr', Trigger)
                rospy.loginfo(f"Kamikaze STARTED AT:")
            ...
            if self.last_rec_wp == 2 and not self.kamikaze_started:
                ...
                try:
                    ...
                    rospy.loginfo(f"Clock STARTED AT: {start_time}")
            ...
            if self.last_rec_wp == 4 and self.kamikaze_started:
                ...
                if qr_text != "":
                    ...
                    response = send_lock_message(qr)
                    return TriggerResponse(success=True)
                else:
                    return TriggerResponse(success=False)

    def abort_kamikaze(self, req):
    ...
    def set_params_for_dive(self, req):
        result = utils.set_params_for_dive()
        return TriggerResponse(success=result)

    def set_params_for_safe_fly(self, req):
        result = utils.set_params_for_safe_fly()
        return TriggerResponse(success=result)

    def get_param(self, req):
        result = utils.get_params()
        return TriggerResponse(success=result)
