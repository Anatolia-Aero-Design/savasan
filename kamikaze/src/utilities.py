...
def get_params():
    ...
    for param in param_list:
        try:
            ...
            response = service_proxy(request)
            if response.success:
                rospy.loginfo(f"Parameter {param} value: {response.value.integer if response.value.integer else response.value.real}")
            else:
                rospy.logerr(f"Failed to get parameter: {param}")
        except rospy.ServiceException as e:
            rospy.logerr(f"Service call failed: {e}")
            return 0
    return 1
