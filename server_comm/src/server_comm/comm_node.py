...
class Comm_Node:
    def __init__(self):
        ...
        self.base_url = rospy.get_param("/comm_node/base_url")
        ...
        self.get_hss_coordinate_message = rospy.Service(
            "get_hss_coordinates", Trigger, self.get_hss_coordinates)
        ...
        self.send_lock_validation = rospy.Service(
            "send_lock_data_to_server", Trigger, self.send_lock_data_validation)
    ...
    def get_qr_cord(self, _):
        ...
        rospy.set_param('comm_node/Target_Lat', response.json().get('qrEnlem'))
        rospy.set_param('comm_node/Target_Lon',
                        response.json().get('qrBoylam'))

    def get_hss_coordinates(self, _):
        try:
            result = self.get_hss_coordinates_process()
            return TriggerResponse(success=result)
        except Exception as e:
            rospy.logerr(f"An error occurred in get_hss_coordinates: {str(e)}")
            return TriggerResponse(success=result)

    def get_hss_coordinates_process(self):
        try:
    ...
    def send_lock_callback(self, data_calback):
        try:
            ...
            self.lock_data = data
            rospy.logwarn(data)
            rospy.wait_for_service("send_lock_data_to_server")
            return sendlockResponse(success=1, result=200)
        except:
            return sendlockResponse(success=0, result=400)

    def send_lock_data_validation(self, _):
        url = f'{self.base_url}/kilitlenme_bilgisi'
        try:
            response = self.session.post(
                url, json=self.lock_data, headers=self.header, timeout=10)

            if response.status_code == 200:
                rospy.logwarn("Lock data send successfully to server")
                rospy.logwarn(self.lock_data)
                return TriggerResponse(success=1)
        except Exception as e:
            rospy.logerr(
                f"An error occurred while sending Lock data to server: {str(e)}")
    ...
    def sent_telem(self):
        ...
        try:
            ...
            rospy.logerr(f"Error during retrieving mode: {self.state}")
            IHA_otonom = 0
        try:
            if self.imu is not None:
                pitch, roll, yaw = quaternion_to_euler(
    ...
    def get_server_time(self):
        try:
            ...
            if response.status_code == 200:
                server_time = response.json()['sunucusaati']
                computer_time = self.get_current_time()
                second_diff = computer_time[2] - server_time['saniye']
                milisec_diff = computer_time[3] - server_time['milisaniye']
...
if __name__ == "__main__":
    ...
    try:
        ...
        rospy.logerr("login not succefull")
    try:
        ...
        rospy.logerr(f"An error occurred in the comm_node main loop: {str(e)}")
