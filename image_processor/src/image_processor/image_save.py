...
class VideoSaveNode:
    def __init__(self):
        ...
        self.tracking_sub = rospy.Subscriber(
            "/camera/image_processed", Image, self.callback)
        ...
        self.video_filename = os.path.join(
            package_path, f"saved_videos/{file_name}")
        ...
        self.start_service = rospy.Service(
            'image_proccessor/start_recording', Trigger, self.start_recording)
        self.stop_service = rospy.Service(
            'image_proccessor/stop_recording', Trigger, self.stop_recording)
    ...
    def stop_recording(self, req):
        try:
            ...
            if self.video_writer is not None:
                ...
                rospy.loginfo(
                    f"Stopped recording. Video saved as {self.video_filename}")
