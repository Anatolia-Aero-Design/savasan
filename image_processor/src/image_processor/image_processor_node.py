...
class ImageProcessorNode:
    ...
    def overlay_server_time(self, frame):
        # Ensure `self.server_time` is a valid string
        if not isinstance(self.server_time, str):
            rospy.logerr("Invalid server time format.")
            return
        
        formatted_time = self.server_time
        
        try:
            # Ensure the text is valid for cv2.getTextSize
            ...
            rospy.logerr(f"An error occurred while overlaying server time: {str(e)}")
