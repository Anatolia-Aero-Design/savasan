  ...
  ui(new Ui::SavasanGui)
{
  ...
  connect(ui->set_param_for_dive, &QPushButton::clicked, this, &SavasanGui::on_set_param_for_diveClicked);
  connect(ui->set_param_for_safe_fly, &QPushButton::clicked, this, &SavasanGui::on_set_param_for_safe_flyClicked);
  connect(ui->getparams, &QPushButton::clicked, this, &SavasanGui::on_getparamsClicked);
  connect(ui->abort_kamikaze, &QPushButton::clicked, this, &SavasanGui::on_abort_kamikazeClicked);
  connect(ui->load_mission, &QPushButton::clicked, this, &SavasanGui::on_load_mission_clicked);
  connect(ui->hss_request, &QPushButton::clicked, this, &SavasanGui::on_hss_request_clicked);
  connect(ui->hss_push, &QPushButton::clicked, this, &SavasanGui::on_hss_push_clicked);
  connect(ui->send_lock_data, &QPushButton::clicked, this, &SavasanGui::on_send_lock_dataClicked);
...
void SavasanGui::on_abort_kamikazeClicked()
{
  ros::NodeHandle nh;
  ros::ServiceClient client = nh.serviceClient<std_srvs::Trigger>("/abort_kamikaze");

  std_srvs::Trigger srv;

  if (client.call(srv)) {
    if (srv.response.success) {
      ROS_INFO("MISSION ABORTED %s", srv.response.message.c_str());
    } else {
      ROS_WARN("Failed to ABORT KAMIKAZE %s", srv.response.message.c_str());
    }
  } else {
    ROS_ERROR("Service call to ABORT KAMIKAZE failed");
  }}
...
void SavasanGui::handleRosLog(const rosgraph_msgs::Log::ConstPtr &msg)
{
  ...
  else if (QString::fromStdString(msg->name) != "/mavros"){
...
void SavasanGui::on_set_param_for_diveClicked()
{
  ros::NodeHandle nh;
  ros::ServiceClient client = nh.serviceClient<std_srvs::Trigger>("/set_params_for_dive");

  std_srvs::Trigger srv;

  if (client.call(srv)) {
    if (srv.response.success) {
      ROS_INFO("Params set successfully for Dive %s", srv.response.message.c_str());
    } else {
      ROS_WARN("Failed to set params for Dive %s", srv.response.message.c_str());
    }
  } else {
    ROS_ERROR("Service call to Set Params For Dive failed!");
  }}

void SavasanGui::on_set_param_for_safe_flyClicked()
{
  ros::NodeHandle nh;
  ros::ServiceClient client = nh.serviceClient<std_srvs::Trigger>("/set_params_for_safe_fly");

  std_srvs::Trigger srv;

  if (client.call(srv)) {
    if (srv.response.success) {
      ROS_INFO("Params set successfully for Safe Flight %s", srv.response.message.c_str());
    } else {
      ROS_WARN("Failed to set params for Safe Flight %s", srv.response.message.c_str());
    }
  } else {
    ROS_ERROR("Service call to Set Params For Safe Flight failed!");
  }}

void SavasanGui::on_getparamsClicked()
{
  ros::NodeHandle nh;
  ros::ServiceClient client = nh.serviceClient<std_srvs::Trigger>("/get_param");

  std_srvs::Trigger srv;

  if (client.call(srv)) {
    if (srv.response.success) {
      ROS_INFO("Params Get successfully %s", srv.response.message.c_str());
    } else {
      ROS_WARN("Failed to Get Params %s", srv.response.message.c_str());
    }
  } else {
    ROS_ERROR("Service call to Get Params failed!");
  }}
...
void SavasanGui::on_send_lock_dataClicked()
{
  ros::NodeHandle nh;
  ros::ServiceClient client = nh.serviceClient<std_srvs::Trigger>("/send_lock_data_to_server");

  std_srvs::Trigger srv;

  if (client.call(srv)) {
    if (srv.response.success) {
      ROS_INFO("Lock data send successfully to server %s", srv.response.message.c_str());
    } else {
      ROS_WARN("Failed to send Lock data to server %s", srv.response.message.c_str());
    }
  } else {
    ROS_ERROR("Service call to send Lock data to server failed!");
  }}
...
void SavasanGui::on_hss_request_clicked()
{
  ros::NodeHandle nh;
  ros::ServiceClient client = nh.serviceClient<std_srvs::Trigger>("/get_hss_coordinates");

  std_srvs::Trigger srv;

  if (client.call(srv)) {
    if (srv.response.success) {
      ROS_INFO("HSS coordinates pulled successfully: %s", srv.response.message.c_str());
    } else {
      ROS_WARN("Failed to get HSS coordinates: %s", srv.response.message.c_str());
    }
  } else {
    ROS_ERROR("Service call to get HSS coordinates failed.");
  }
}

void SavasanGui::on_hss_push_clicked()
{
  ros::NodeHandle nh;
  ros::ServiceClient client = nh.serviceClient<std_srvs::Trigger>("/push_hss_coordinates");

  std_srvs::Trigger srv;

  if (client.call(srv)) {
    if (srv.response.success) {
      ROS_INFO("HSS coordinates pushed successfully: %s", srv.response.message.c_str());
    } else {
      ROS_WARN("Failed to push HSS coordinates: %s", srv.response.message.c_str());
    }
  } else {
    ROS_ERROR("Service call to push HSS coordinates failed.");
  }
}
